...
### 6.3 運用のコツ
- **フック（最初の1〜2行）が9割**。結論・数字・意外性を先頭に。
- **投稿頻度**: 毎日1〜複数。継続が最大の変数。
- **エンゲージメント**: 一方通行で売らず、リプ/引用で関係を作る。質問・実例募集は反応が出やすい。
- **保存・シェアされる設計**（一覧化、チェックリスト、図解）はリーチが伸びやすい。
- **直リンクの扱い**: 本文に外部リンクを貼るとリーチが落ちやすいとの実務的経験則がある。リンクはプロフ/リプ/「詳細はブログ」誘導に寄せるのが無難（仕様は変動。要検証）。
- **広告/PRの明示は必須**（§10）。アフィリンクを含む発信は「PR」「広告」を明確に。

### 6.4 X発信のNG（やってはいけない）
- 収益スクショの捏造・盛り、"誰でも簡単に月◯万"の断定
- フォロワー購入、相互フォロー乱用、無断転載画像
- PR表記の欠落（ステマ規制違反）
...
### 9.1 信頼できるサイン（加点）
- [ ] **長期運用**の実体（数か月〜数年の継続、更新履歴）
- [ ] **再現性のある具体手法**を開示（抽象的な精神論でない）
- [ ] **一次情報**（実際の管理画面以外の裏付け、製品の実使用、スクショ・手順）
- [ ] **失敗や数字の停滞も正直に**語る
- [ ] **収益源が健全**（物販・サービス送客が中心で、"稼ぐ系商材販売"一辺倒でない）
- [ ] 第三者からの言及・被リンク・指名検索がある
- [ ] **PR/広告表記**を適切に行っている

### 9.2 怪しいサイン（減点・除外）
- [ ] 収益スクショの**加工・使い回し**、出所不明の「月◯万円」
- [ ] 「**誰でも・簡単に・すぐ**」の断定、煽り
- [ ] 収益が**"稼ぐ系"の教材・サロン・コンサル販売に偏る**（＝ノウハウを売るのが本業）
- [ ] **再現性のない**成功体験、具体手順の欠如
- [ ] フォロワー購入・エンゲージ操作の形跡
- [ ] 実態のない**権威付け**（肩書きの誇張）
- [ ] PR表記の欠落・ステマ的手法の推奨

> **判定ルール（エージェント用）**: 9.2 に2つ以上該当する発信者は手本から除外する。9.1 を満たす発信者・一次情報・ASP公式・公的機関を優先採用する。
