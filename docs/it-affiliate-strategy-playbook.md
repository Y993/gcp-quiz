...
### 10.1 ステマ規制（景品表示法・2023/10/1施行）
- 該当する2要件：**①事業者が表示内容の決定に関与（対価・商品の無償提供・アフィリエイト提携など便益を受ける）**、かつ**②一般消費者に事業者の表示だと判別困難**な表示が違反。
- **対策**: アフィリエイト/PR案件を含む発信には「**広告**」「**PR**」「**プロモーションを含みます**」等を**冒頭・目立つ位置**に明示。**アフィリリンクの有無を問わない**（提携商品の紹介投稿はリンクが無くても表記が必要になり得る）。
- **NG（不適切例）**: 大量ハッシュタグ末尾に「#PR」を紛れ込ませる／極小文字・折りたたみ（「続きを読む」内）に隠す／動画でごく短時間だけ表示／一般消費者が分からない業界用語・外国語で表記。
- **最重要の落とし穴**: **SNS原本にPR表記があっても、転載・二次利用（自社ショップ等への再掲）で表記が外れると違反**になる〔実例：大正製薬 2024/11、ロート製薬 2025/3 の措置命令〕。全媒体で表記を維持する。
- **責任主体**: 措置命令の対象は供給事業者（広告主）だが、違反が出れば**提携解除・報酬停止**になるため、発信者も遵守必須。

### 10.2 景品表示法（優良誤認・有利誤認）
- **優良誤認**: 品質・内容を実際より著しく優良に見せる表示は禁止。**有利誤認**: 価格・取引条件を実際より著しく有利に見せる表示は禁止。
- **No.1・最上級表示**（「業界No.1」「満足度No.1」「最安」等）は**合理的根拠（調査対象・方法・時点の出典）が必須**。根拠なしは不可（消費者庁「No.1表示実態調査報告書」2024/9で取締り強化）。
- **打消し表示**（「効果には個人差があります」等）は、**強調表示と同等の大きさ・近接した位置**に置く。極小文字・離れた位置はNG。
- **2024/10/1の改正施行**: 確約手続の導入、課徴金の割増（10年内の再違反は1.5倍）、**直罰（優良/有利誤認に罰金100万円以下）**を新設。

### 10.3 薬機法（医薬品医療機器等法）
- **「何人も」が規制対象＝個人アフィリエイター・ブロガーも違反主体**になり得る（66条 誇大広告・68条 未承認広告）。「個人の感想です」では免責されない。
- 健康食品・サプリで**「治る・予防・改善・痩せる・免疫力アップ」等の医薬品的効能効果を標榜しない**。化粧品は**厚労省の56項目の効能効果の範囲内のみ**（「シワが消える」等の逸脱は事実でも違反）。
- **課徴金**: 誇大広告（66条違反）は対象売上額の**4.5%**（2021年導入）。
- **IT系の注意**: 健康ガジェット・ウェアラブル・美顔器等で、医療機器認証なく「血圧を下げる」「治療する」等の効能を謳わない。

### 10.4 各ASP・プラットフォーム規約
- **共通**: 誇大/虚偽表現の禁止、無断コピー禁止、不正クリック/不正申込の禁止。案件ごとにリスティング可否・禁止キーワードを確認。
- **商標リスティング**: 「楽天」等のサービス名・第三者の商標をキーワード指定するリスティング広告は原則禁止。
- **自己アフィリ（セルフバック/セルフB）**: 広告主が許可したプログラムのみ。報酬目的だけの申込・繰り返し申込・転売/第三者名義はNG。楽天は自己アフィリの直接遷移を禁止。
- **Amazonアソシエイト**: ①価格/在庫は**公式ツール（PA-API/リンク作成ツール）経由**で表示（手動表記は原則不可）、②画像は**公式提供画像のみ・スクショ/加工はNG**、③サイト/SNSに**「アソシエイト・プログラム参加者である」旨の明示**が必須。
- **楽天**: ガイドライン・素材利用ルールに従う。

### 10.5 著作権・引用（著作権法32条）
- 適法な引用の5要件を**すべて**満たす：①**明瞭区別性**（blockquote等で区分）②**主従関係**（自分が主・引用が従）③**必然性**④**無改変**⑤**出所明示**。**出典を書くだけでは不十分**（主従関係・明瞭区別を欠けば無断転載）。
- 画像・スクショの無断転載は原則NG。ただしスクショも**5要件を満たせば引用たり得る**（X投稿スクショに関する裁判例）。画像はライセンス確認（公式提供/CC/購入素材）を。

### 10.6 出典（最新を確認）
- 消費者庁 ステマ規制: https://www.caa.go.jp/policies/policy/representation/fair_labeling/stealth_marketing ／ Q&A: https://www.caa.go.jp/policies/policy/representation/fair_labeling/faq/stealth_marketing/
- 消費者庁 大正製薬への措置命令（転載でPR表記が外れた例）: https://www.caa.go.jp/notice/entry/039990/
...
- A8.net コンプライアンス: https://www.a8.net/compliance/a8compliance.php ／ 楽天アフィリエイト規約: https://affiliate.rakuten.co.jp/guideline/rule/ ／ Amazonアソシエイト運営規約: https://affiliate.amazon.co.jp/help/operating/agreement/
...
- **追補（2回目の調査）**: 初回にレート制限で未完だった「X発信（§6）」「本物 vs 見せかけ判定（§9）」「コンプライアンス（§10）」を、後日**1体ずつ逐次で再調査**し、公的機関（消費者庁・国民生活センター・警察庁・金融庁・厚労省）や各社公式を出典に**裏取り済みの内容へ差し替え**ました。
